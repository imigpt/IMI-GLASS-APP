...
class HeyImiWakeWordDetector(
    ...
    companion object {
        ...
        // 🔧 TUNED: 0.40f - MORE SENSITIVE for better detection
        const val DEFAULT_THRESHOLD = 0.40f
        
        // Minimum audio level required before checking classifier
        // 🔧 TUNED: 500 - RELAXED to catch soft voices
        // This is the KEY to preventing false positives from ambient noise
        private const val MIN_AUDIO_LEVEL_FOR_DETECTION = 500
    ...
    // 🔧 RELAXED: Detection consistency - allow more flexibility
    private const val CONFIDENCE_THRESHOLD_FOR_DETECTION = 0.35f  // Raw score must be at least this
    private const val CONSECUTIVE_FRAMES_NEEDED = 1  // Need 1+ consistent frames (relaxed from 2)}
    ...
    private val SCORE_WINDOW_SIZE = 5  // 🔧 5 frames - enough history for consecutive check
    ...
    /**
     * Smoothing: Slight peak awareness + mostly average
     * Real wake words sustain high avg, noise spikes don't
     */
    private fun smoothScore(score: Float): Float {
        ...
        // 🔧 30% peak + 70% avg: Mostly average, slight peak for real wake words
        return 0.3f * peak + 0.7f * avg
    }
            ...
            try {
                ...
                if (readSize > 0) {
                    ...
                    if (finalScore > threshold && (now - lastDetectionTime) > DETECTION_COOLDOWN_MS) {
                        // 🔧 RELAXED: Raw score just needs to be reasonably high
                        if (confidence < CONFIDENCE_THRESHOLD_FOR_DETECTION) {
                            Log.d(TAG, "⚠️ Skip: Raw score too low ($confidence < $CONFIDENCE_THRESHOLD_FOR_DETECTION)")
                            continue
                        }
                        
                        // 🔧 RELAXED: Need 1+ high-confidence frame (not 2)
                        val recentScores = scoreWindow.toList()
                        val lastN = if (recentScores.size >= 2) recentScores.takeLast(2) else recentScores
                        val consistentCount = lastN.count { it > CONFIDENCE_THRESHOLD_FOR_DETECTION }
                        if (consistentCount < CONSECUTIVE_FRAMES_NEEDED) {
                            Log.d(TAG, "⚠️ Skip: Need $CONSECUTIVE_FRAMES_NEEDED consistent frames, got $consistentCount [${lastN.joinToString(",") { "%.2f".format(it) }}]")
                            continue
                        }
                        
                        lastDetectionTime = now
                        
                        val consecutiveStr = lastN.joinToString(",") { "%.2f".format(it) }
                        Log.i(TAG, "🔥 Hey IMI DETECTED! smooth=$finalScore (raw=$confidence) [amp=$maxAmp] [frames=$consecutiveStr]")
    ...
    private val TARGET_LEVEL = 0.85f          // 85% target volume
    private val MAX_GAIN = 12.0f              // 🔧 12x max gain (soft voices work, noise filtered by MIN_AUDIO_LEVEL)
    private val MIN_GAIN = 1.0f               // Minimum gain (Never attenuate)
    private val NOISE_GATE_THRESHOLD = 0.05f  // 🔧 STRICT: Mute quiet samples to prevent amplified noise
    ...
    private fun preprocessAudio(audioBuffer: ShortArray): FloatArray {
        ...
        for (i in audioBuffer.indices) {
            ...
            // Noise Gate - ENABLED to prevent false positives from amplified silence
    ...
    private fun getClassifierOutput(result: OrtSession.Result?): Float {
        ...
        return try {
            ...
            // 🔧 ENHANCED LOGGING: Show detailed detection stats
            if (prob > 0.1f) {
                Log.d(TAG, "🎯 Classifier: raw_logit=$rawLogit → prob=${String.format("%.4f", prob)} threshold=${"%.2f".format(threshold)}")
            }
    ...
    fun processExternalAudio(pcmData: ByteArray) {
        ...
        try {
            ...
            synchronized(bufferLock) {
                ...
                if (finalScore > threshold && (now - lastDetectionTime) > DETECTION_COOLDOWN_MS) {
                    // 🔧 RELAXED: Just need 1 frame above threshold
                    val consistentFrames = scoreWindow.count { it > CONFIDENCE_THRESHOLD_FOR_DETECTION }
                    if (consistentFrames < CONSECUTIVE_FRAMES_NEEDED) {
                        return  // Not enough consistent frames
                    }
